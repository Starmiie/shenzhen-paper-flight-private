...
test('Wing trails stay bounded, expire at rest, and clear in photo mode',()=>{
 ...
 for(const mesh of scene.meshes){assert.equal(mesh.getTotalVertices(),4096);assert.ok(mesh.getVerticesData(VertexBuffer.ColorKind)!.some((v,i)=>i%4===3&&v>0));assert.ok(mesh.getVerticesData(VertexBuffer.PositionKind)!.every(Number.isFinite));}
 for(let i=0;i<180;i++)trail.update(1/60,plane,camera,false,false);
